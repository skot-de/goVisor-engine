...
**Changelog:** 1.5 Erstfassung (verteilte Erhebung, Reziprozität, Kartell-Leitplanken) · 1.6 Kaltstart-Auflösung ergänzt — Eigenwert der eigenen Meldung vor der Reziprozität, Bootstrapping-Reihenfolge
...
Die Reziprozitätsachse. Hier wird der Handel offen benannt — aber erst, nachdem die eigene Meldung schon für sich einen Wert hat (siehe 4.3.0). Die Reihenfolge ist entscheidend, nicht kosmetisch.

#### 4.3.0 Der Eigenwert zuerst — die Kaltstart-Auflösung

Das zentrale Risiko des ganzen Ergebnisdaten-Grabens: Niemand meldet, um eine Wettbewerbsmenge zu sehen, die noch leer ist — aber die Menge *ist* die Summe der Meldungen. Ein Henne-Ei, an dem jedes „gib-um-zu-sehen"-Netzwerk (Glassdoor, PitchBook) jahrelang gelitten hat. Wenn Block 3 die Meldung nur als Eintrittspreis für einen anfangs leeren Pool darstellt, meldet die erste Kohorte nicht — und der Graben entsteht nie.

**Die Auflösung: Die eigene Meldung nützt der Firma sofort und allein, unabhängig davon, ob je ein anderer beiträgt.** Der Pool ist der zweite Nutzen, nicht der erste. Was eine Ergebnismeldung dem Melder *ohne jeden Gegenpool* gibt:

```
  Eure Teilnahmen                            unabhängig von anderen

  Ein privater Überblick über eure Bewerbungen — beworben,
  gewonnen, verloren, mit Grund. Nur für euch sichtbar.

  ○ Offene Fristen im Blick          erinnert an eure laufenden Verfahren
  ○ Eure eigene Bilanz               Quote, Volumen, Muster — nur eure
  ○ Nachweis für die Erfolgsprämie   sauber dokumentiert, falls relevant
  ○ Lernschleife bei Verlust         wo lag es, wo ist die nächste Chance
```

Das ist ein privates Bewerbungs-Tracking — nützlich am ersten Tag, für den ersten Nutzer, mit null anderen im System. Es braucht keine Freischaltstufe, keine Mindestzahl, keinen fremden Beitrag. Es ist der Grund, überhaupt zu melden.

**Erst darauf setzt die Reziprozität auf** — als Verstärker, nicht als einzige Motivation. So war es auch bei Glassdoor: Sein Gehalt einzutragen war ein sinnvoller Akt für sich; die „gib-um-zu-sehen"-Mechanik hat ihn verstärkt, nicht erst erzeugt. Aus dem Henne-Ei wird ein Bootstrapping.

**Konsequenz für den Bau:** Das private Tracking (`user_outcomes` als eigene, private Bilanz) muss **vor** der Aggregatsanzeige gebaut und im Interface **vor** der Reziprozitätsstufe gezeigt werden. Die Reihenfolge in Abschnitt 18 spiegelt das.

#### 4.3.1 Die Reziprozität als zweite Stufe
...
Wichtig für die Frühphase: Solange der Pool dünn ist, bleibt der Aggregat-Teil ehrlich leer oder ausgegraut („noch nicht genug Meldungen im Markt") — aber das private Tracking aus 4.3.0 funktioniert trotzdem. Der Nutzer verliert nie, er gewinnt nur später zusätzlich. Kein Versprechen, das ein leeres System bricht.
...
Die Bootstrapping-Logik aus 4.3.0 prägt die Reihenfolge: Der **Eigenwert der eigenen Meldung** (privates Tracking) kommt vor jeder Reziprozität und vor jedem Aggregat. Schritt 5 und 6 sind bewusst getrennt.
...
| 5 | `user_outcomes` + Ergebnisdialog aus dem Alert | Datenbasis der Meldung |
| 6 | **Privates Tracking (Block 3, Teil 4.3.0)** | **Der Eigenwert — nützt ab dem ersten Nutzer, ohne Gegenpool. Löst den Kaltstart.** |
| 7 | Reziprozitätsstufen (Block 3, Teil 4.3.1) | Verstärker — erst sinnvoll, wenn Meldungen fließen |
| 8 | `user_contracts` | Eigenständiger Nutzen, unabhängig vom Rest |
| 9 | Aggregate und Rückspiel | Nach kartellrechtlicher Prüfung, ab Mindestzahl |

Der kritische Punkt: Schritt 6 (privates Tracking) muss **vor** Schritt 7 (Reziprozität) live sein. Wird die Reihenfolge vertauscht — Reziprozität zuerst — steht die erste Nutzer-Kohorte vor einer Bezahlschranke aus Meldungen für einen leeren Pool und meldet nicht. Genau der Kaltstart, den 4.3.0 auflöst.
