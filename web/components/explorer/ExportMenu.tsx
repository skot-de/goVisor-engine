...
const bandWort = (b?: string) => (!b || b === "na" ? "" : b);
...
const COLS: { label: string; get: (l: Lead) => string }[] = [
  ...
  { label: "Volumen-Quelle", get: (l) => ({ echt: "veröffentlicht", schaetz: "geschätzt", unbekannt: "unbekannt" }[l.volumen?.src as string] || "") },
  { label: "Vertragsart", get: (l) => l.art || "" },
  { label: "Rechtsrahmen", get: (l) => (l.rahmen ? String(l.rahmen).toUpperCase() : "") },
  { label: "Leistungsart", get: (l) => ({ dienst: "Dienstleistung", liefer: "Lieferung", bau: "Bauleistung" }[l.naturKat as string] || "") },
...
export function ExportMenu({ rows, view }: { rows: Lead[]; view: string }) {
  ...
  return (
    <div className="colcfg" ref={ref} style={{ position: "relative" }}>
      <button className="colbtn" type="button" onClick={() => setOpen((o) => !o)} disabled={!rows.length} title="Aktuelle Liste exportieren">
      ...
      {open && (
        <div className="exp-menu">
          <button className="exp-opt" onClick={() => run("xlsx")}>
            <b>Excel</b><span>.xlsx · formatiert</span>
          </button>
          <button className="exp-opt" onClick={() => run("csv")}>
            <b>CSV</b><span>.csv · UTF-8, Semikolon</span>
          </button>
          <div className="exp-note">{rows.length > MAX ? `${MAX} von ${rows.length} (Obergrenze)` : `${rows.length} Leads · aktuelle Ansicht`}</div>
        </div>
      )}
    </div>
  );
