...
**Das Kaltstartproblem — ehrlich, weil es das Kernrisiko der ganzen Graben-These ist.** Der Satz „Du warst dabei? Melde das Ergebnis" klingt, als geschähe der Beitrag von selbst. Tut er nicht. Es ist ein Henne-Ei-Problem: Niemand meldet seine Ergebnisse, um eine Wettbewerbsmenge zu sehen, die noch leer ist — aber die Menge *ist* die Summe der Meldungen. Jedes „gib-um-zu-sehen"-Netzwerk (Glassdoor, PitchBook) hat jahrelang daran gelitten.

Die Auflösung: Der **erste Beitrag darf nicht vom Pool abhängen**. Die eigene Biethistorie muss der meldenden Firma *allein* nützen — privates Tracking der eigenen Teilnahmen, Fristerinnerung, Nachweis für die Erfolgsprämie —, unabhängig davon, ob schon jemand anderes beigetragen hat. Erst aus diesem Eigennutzen wird aggregiert. Das ist die Bootstrapping-Bedingung, die vor dem Graben gebaut werden muss, und sie gehört konkret in die Treffergüte-Umsetzung (Ticket #11): zuerst der Eigenwert der eigenen Meldung, dann die Reziprozität. Ohne diesen Schritt bleibt der Graben ein Versprechen.
