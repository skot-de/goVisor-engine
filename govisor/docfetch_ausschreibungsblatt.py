...
def lauf(limit: int | None = None, dry_run: bool = False, country: str = "DE") -> dict:
    ...
    with _queue.Wache("ausschreibungsblatt", vorgang_hart_s=0, leerlauf_s=LEERLAUF_S,
        ...
        for i, (lead_id, url, ziel) in enumerate(offen, 1):
            ...
            if r["status"] == "downloaded":
                for zwilling in geschwister.get(url, []):
                    # ⚠ Atomar: ein Abbruch mitten in der Kopie hinterliesse sonst ein
                    # halbes ZIP unter dem richtigen Namen (s. `_queue.schreibe_atomar`).
