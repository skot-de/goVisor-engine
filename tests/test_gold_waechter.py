...
def test_neues_land_wird_von_allein_mitgeprueft(waechter, tmp_path):
    """Der eigentliche Zweck von `_laender`: ein neues Land darf niemand eintragen muessen.

    ⚠ HIER STAND „PL" — und das war beim Zusammenfuehren am 2026-09-15 falsch geworden.
    `pruefe_verdrahtung._laender` nimmt seit dem 2026-09-02 `BEWUSST_OHNE_GOLD` aus, und
    Polen steht genau dort („angefangen und liegengeblieben"). Der Test nahm also
    ausgerechnet das Land als Beispiel fuer „wird automatisch mitgeprueft", das
    ausdruecklich NICHT mitgeprueft wird.

    Die Pruefabsicht bleibt; nur das Beispiel muss eines sein, das keine Ausnahme ist.
    `PT` ist frei und wird es bleiben, solange es niemand aufnimmt — und nimmt es jemand
    auf, faellt dieser Test auf und man waehlt das naechste.
    """
    from importlib.util import module_from_spec, spec_from_file_location
    _s = spec_from_file_location("pv_ausnahmen", ROOT / "scripts" / "pruefe_verdrahtung.py")
    _pv = module_from_spec(_s)
    _s.loader.exec_module(_pv)
    assert "PT" not in _pv.BEWUSST_OHNE_GOLD, (
        "PT steht inzwischen in BEWUSST_OHNE_GOLD — dieser Test braucht ein Land ohne "
        "Ausnahme, sonst prueft er das Gegenteil seiner Absicht")

    _gold(tmp_path, "PT", "quality.parquet", {"notice_id": ["pt-1"]})
    _paar_tabelle(tmp_path, "PT", master=["pt-1"], dup=["pt-9"])
    assert "PT" in waechter._laender()
